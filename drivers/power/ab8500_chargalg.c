...
static int ab8500_chargalg_check_charger_connection(struct ab8500_chargalg *di)
{
	if (di->chg_info.conn_chg != di->chg_info.prev_conn_chg ||
		 ...
		 */
		/* dev_samsung_dbg is completely invalid due to new configuration, hence for every instance generic linux dev_dbg must be used to outcast the messages to the console*/
